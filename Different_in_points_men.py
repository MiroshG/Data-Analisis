...
def prob_winning_men():
    ...
    plt.xlabel('Overall points difference in the first two rounds')
...
def hypotesis_testing(men_dict, women_dict):
    ...
    plt.figure()
    plt.scatter(key_men, values_men, label='MEN')
    plt.scatter(key_women, values_women, label='WOMEN')
    plt.xlabel('Overall points difference in the first two rounds')
    plt.legend()
    plt.grid(False)
    plt.title('Overall difference in points distribution')
    plt.savefig('Overall difference in points distribution.png', dpi=300, bbox_inches='tight')
